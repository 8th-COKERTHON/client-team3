...
function DifficultyBar({ difficulty, filledClass }: { difficulty: number; filledClass: string }) {
  return (
    <div className="flex items-center gap-[3px]">
      {Array.from({ length: DIFFICULTY_MAX }, (_, i) => (
        <span key={i} className={`h-1 w-1 rounded-full ${i < difficulty ? filledClass : "bg-gray-200"}`} />
      ))}
    </div>
  );
}
    ...
    <div className="flex items-center gap-3 py-3.5">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-body-02 leading-none font-medium text-gray-400">{chore.assigneeName}</span>
          <span className="text-body-02 leading-none text-gray-300">·</span>
          <DifficultyBar difficulty={chore.difficulty} filledClass={bgClass} />
          <span className={`text-body-02 leading-none font-semibold ${textClass}`}>+{chore.point}pt</span>
        </div>
      </div>
      ...
      >
        {chore.done ? <CheckCircle2 size={22} strokeWidth={1.5} /> : <Circle size={22} strokeWidth={1.5} />}
      </button>
    </div>
  );
}
